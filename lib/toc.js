...
Toc.prototype.generate = function(callback){
    ...
    for(var i=0; i<childs.length; i++){
        ...
        if((childs[i].length > 3) && (childs[i].substr(-3) == ".md")){
            this.addItem({
...
Toc.prototype.toHTML = function(opts){
    ...
    if(this.children.length > 0){
        ...
        if(opts.title && (opts.title !== false)){
            ...
            var subItems = "";
            for(var i=0; i<this.children.length; i++){
                if(this.children[i].children.length == 0) subItems += this.children[i].toHTML(opts.selected);
            }
            ...
            for(var i=0; i<this.children.length; i++){
                if(this.children[i].children.length > 0) s += this.children[i].toHTML(opts.selected);
            }

        } else {

            for(var i=0; i<this.children.length; i++){
                s += this.children[i].toHTML(opts.selected);
            }
...
TocItem.prototype.fromFile = function(filePath, rootPath){
    ...
    if(stats.isDirectory()) {
        mdPath = path.join(filePath, 'readme.md');
    }
...
TocItem.prototype.toHTML = function(selected){
    var s = "";

    if(!this.hidden){

        if(selected && (this.link == selected)){
            s += '<li class="';
        ...
        if(this.isLink){
            s += '<a href="'+this.link+'">'+this.title+'</a>';
        } else {
            s += this.title;
        }
        ...
        if(this.children.length > 0){
            ...
            for(var i=0; i<this.children.length; i++){
                s += this.children[i].toHTML(selected);
            }
...
function getTocItemInfoFromFilePath(filePath){
    ...
    // info.title = info.title.replace(/\w\S*/g, function(a){return a.charAt(0).toUpperCase() + a.substr(1).toLowerCase();});
    console.log(filePath, info.title);
...
function getRelativeUrl(filePath, rootPath){
    ...
    if((relativeUrl.length > 3) && (relativeUrl.substr(-3) == ".md")){
        relativeUrl = relativeUrl.substr(0, relativeUrl.length-3);
        if((relativeUrl.length > 6) && (relativeUrl.substr(-6) == "readme"))
